...
namespace MIDI
{
    ...
    public class SynthesisEngine : ISynthesisEngine
    {
        ...
        {
            this.config = config;
            this.editorSettings = MidiEditorSettings.Default;
        ...
        public double GetFrequency(int noteNumber, double pitchBend, int centOffset, double pitchLfo = 0.0)
        {
            var semitoneOffset = pitchBend + pitchLfo;
            double baseFreq;

            switch (editorSettings.TuningSystem)
            {
                case TuningSystemType.TwentyFourToneEqualTemperament:
                    baseFreq = config.Synthesis.A4Frequency * Math.Pow(2.0, (noteNumber - 69.0 * 2.0) / 24.0);
                    break;
                case TuningSystemType.JustIntonation:
                    int octave = noteNumber / 12;
                    int noteInOctave = noteNumber % 12;
                    double[] justIntonationRatios = { 1.0, 16.0 / 15.0, 9.0 / 8.0, 6.0 / 5.0, 5.0 / 4.0, 4.0 / 3.0, 45.0 / 32.0, 3.0 / 2.0, 8.0 / 5.0, 5.0 / 3.0, 9.0 / 5.0, 15.0 / 8.0 };
                    baseFreq = config.Synthesis.A4Frequency * Math.Pow(2.0, octave - 4) * justIntonationRatios[noteInOctave];
                    break;
                case TuningSystemType.Microtonal:
                    semitoneOffset += (centOffset / 100.0);
                    baseFreq = config.Synthesis.A4Frequency * Math.Pow(2.0, (noteNumber - 69.0 + semitoneOffset) / 12.0);
                    break;
                case TuningSystemType.TwelveToneEqualTemperament:
                default:
                    baseFreq = config.Synthesis.A4Frequency * Math.Pow(2.0, (noteNumber - 69.0 + semitoneOffset) / 12.0);
                    break;
            }
        ...
        public float GenerateWaveform(WaveformType type, double frequency, double time, float amplitude, double envelope, int noteNumber, string? wavetableFile = null)
        {
            ...
            if (config.Synthesis.EnableBandlimitedSynthesis)
            {
                wave = type switch
                {
                    ...
                    WaveformType.Noise => (threadRandom.Value!.NextDouble() * 2 - 1),
        ...
        {
            double value = 2.0 * (phase - Math.Floor(phase)) - 1.0;
        ...
        private double GenerateBandlimitedTriangle(double phase, double phaseIncrement)
        {
            double saw = GenerateBandlimitedSawtooth(phase, phaseIncrement);
            double leakyIntegratorFactor = 0.999;
            double previousOutput = 0;
            double currentOutput = (saw + previousOutput * leakyIntegratorFactor) * (1.0 - leakyIntegratorFactor);
            previousOutput = currentOutput;

            return 4.0 * currentOutput - 1.0;
        }
        ...
        private double GenerateUserWavetable(double phase, string? wavetableFile)
        {
            ...
            i1 = ((i1 % table.Length) + table.Length) % table.Length;
            i2 = ((i2 % table.Length) + table.Length) % table.Length;
        ...
        private float GenerateKarplusStrong(int noteNumber, double frequency, double time)
        {
            ...
            if (!karplusStrongBuffers.ContainsKey(noteNumber) || isNoteStart)
            {
                ...
                {
                    buffer.Enqueue((float)(threadRandom.Value!.NextDouble() * 2 - 1));
                }
                karplusStrongBuffers[noteNumber] = buffer;
            }

            if (!karplusStrongBuffers.TryGetValue(noteNumber, out var currentBuffer) || currentBuffer.Count == 0)
                return 0;
            ...
            if (currentBuffer.Count > 0)
                nextValue = (currentValue + currentBuffer.Peek()) * 0.5f * 0.996f;
            else
                nextValue = currentValue * 0.5f * 0.996f;
        ...
        public double GetLfoValue(LfoSettings lfo, double time)
        {
            ...
            return lfo.Waveform switch
            {
                ...
                LfoWaveformType.Noise => (threadRandom.Value!.NextDouble() * 2 - 1),
        ...
        public double GenerateBasicWaveform(string waveType, double frequency, double time)
        {
            ...
            return waveType.ToLower() switch
            {
                ...
                "noise" => (threadRandom.Value!.NextDouble() * 2 - 1),
                _ => Math.Sin(phase)
            };
        }
        ...
        public InstrumentSettings GetInstrumentSettings(int channel, int program, Dictionary<int, InstrumentSettings> instrumentSettings)
        {
            ...
            return instrumentSettings.TryGetValue(key, out var settings) ? settings : GetDefaultInstrumentSetting();
        }

        private InstrumentSettings GetDefaultInstrumentSetting()
        {
            return new InstrumentSettings
            {
                WaveType = Enum.TryParse<WaveformType>(config.Synthesis.DefaultWaveform, true, out var waveType) ? waveType : WaveformType.Sine,
                Attack = config.Synthesis.DefaultAttack,
                Decay = config.Synthesis.DefaultDecay,
                Sustain = config.Synthesis.DefaultSustain,
                Release = config.Synthesis.DefaultRelease,
        ...
        public Dictionary<int, InstrumentSettings> InitializeInstrumentSettings()
        {
            ...
            foreach (var preset in config.InstrumentPresets)
            {
                ...
                if (!Enum.TryParse<WaveformType>(preset.Waveform, true, out waveType))
                {
                    waveType = WaveformType.Sine;
                }
                FilterType filterType;
                if (!Enum.TryParse<FilterType>(preset.Filter.Type, true, out filterType))
                {
                    filterType = FilterType.None;
                }
                ...
                for (int i = preset.StartProgram; i <= preset.EndProgram; i++)
                {
                    settings[i] = new InstrumentSettings
                    {
                        ...
                        FilterLfo = (LfoSettings)preset.Filter.Lfo.Clone(),
                    };
                }
            }
            ...
            for (int i = 0; i <= 127; ++i)
            {
                if (!settings.ContainsKey(i))
                    settings[i] = GetDefaultInstrumentSetting();
            }
        ...
        {
            WaveformType waveType;
            if (!Enum.TryParse<WaveformType>(customInst.Waveform, true, out waveType))
            {
                waveType = WaveformType.Sine;
            }
            FilterType filterType;
            if (!Enum.TryParse<FilterType>(customInst.Filter.Type, true, out filterType))
            {
                filterType = FilterType.None;
            }
            ...
            return new InstrumentSettings
            {
                ...
                FilterLfo = (LfoSettings)customInst.Filter.Lfo.Clone(),
                PitchLfo = (LfoSettings)customInst.PitchLfo.Clone(),
                AmplitudeLfo = (LfoSettings)customInst.AmplitudeLfo.Clone()
            };
        }
    }
}