...
{
    public partial class AmpSimulatorControl : UserControl, IPropertyEditorControl
    {
        public event EventHandler? BeginEdit;
        public event EventHandler? EndEdit;
        private ItemProperty[] itemProperties = [];
        private AmpSimulatorViewModel? viewModel;
        private readonly DispatcherTimer timer;
        private static readonly Regex _regex = new Regex("[^0-9.-]+");

        public AmpSimulatorControl()
        {
            InitializeComponent();
        ...
        public void SetItemProperties(ItemProperty[] properties)
        {
            ...
            if (itemProperties.Length > 0 && itemProperties[0].PropertyOwner is AmpSimulatorAudioEffect owner)
            {
                viewModel = new AmpSimulatorViewModel { EffectItem = owner };
