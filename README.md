...
3. YMM4にある「設定」→「MIDI 設定」から、音源の種類やエフェクトなどの詳細設定が可能です。
...
### リリースノート
[2025/11/26] v8.2.0
- [追加] MIDIエディタ - ショートカットキーのカスタマイズ:
    - ユーザーが好みに合わせてキーボードショートカットを変更・割り当てできる機能を追加しました。
- [追加] MIDIエディタ - 視認性強化:
    - 黒鍵盤・白鍵盤の行の色の反転を行い、ノートの配置位置を直感的に識別できるようデザインを調整しました。
    - 1小節ごとに実線のグリッドラインを追加し、リズムの把握や編集作業を容易にしました。
- [追加] MIDI - レンダリングモードの追加:
    - 新しいレンダリングモードを追加し、出力やプレビューの用途に応じた柔軟な選択が可能になりました。
- [追加] MIDI - 分散処理機能の追加:
    - ネットワークを介した分散処理機能を追加しました。複数のリソースを活用することで、負荷の高い処理の効率化を図っています。
- [修正] MIDI - 新規作成時のタイミングずれの解決:
    - MIDIファイル新規作成時に開始時間がずれる問題を解決しました。
- [修正] MIDI - ymidi読み込み時の例外発生の解決:
    - ymidiファイル読み込み時、新規作成ファイルで例外が発生する問題を解決しました。
- [改善] プリセットの挙動変更:
    - プリセットの保存および適用時の挙動を変更し、より直感的で管理しやすい動作に改善しました。

[2025/11/24] v8.1.2
- [修正] MIDI ピアノロール(図形)の鍵盤表示領域の修正:
    - 鍵盤と背景の描画処理において、描画処理のループ範囲を、パラメーターで設定された minNote から maxNote の値に厳密に一致するように修正しました。
	
[2025/11/23] v8.1.1
-  [修正] タイムルーラーの挙動修正:
    - 水平ズーム・垂直ズームで入力待ち状態でスライダーを動かした際に解除されない問題を修正しました。
    - 水平ズームの場合、縮小時に時間表示が消えてしまう問題を修正しました。
	
[2025/11/23] v8.1.0
- [修正] プラグイン名の統一:
    - 音声エフェクトの名前を英語に統一しました。
- [修正] MIDIエディタの描画エンジンの修正:
    - ビットマップ形式へ移行しました。
- [追加] エディタモードの実装:
    - MIDIエディタのタイムライン操作の挙動変更を可能にしました。
- [追加]  新しいオーディオエフェクトの追加:
    - マルチバンド・サチュレーター/エンハンサーを追加しました。
